...
$searchSql = "SELECT up.first_name, up.last_name, s.title AS specialty_title, cp.application_status, cp.ranking_position, cp.points, cp.created_at FROM candidate_profiles cp INNER JOIN users u ON u.id = cp.user_id
        INNER JOIN user_profiles up ON up.user_id = u.id LEFT JOIN specialties s ON s.id = cp.specialty_id WHERE 1 = 1";
$searchTypes = '';
$searchParams = [];
if ($searchName !== '') {
    $searchSql .= " AND (up.first_name LIKE ? OR up.last_name LIKE ? OR CONCAT(up.first_name, ' ', up.last_name) LIKE ? OR CONCAT(up.last_name, ' ', up.first_name) LIKE ?)";
    $searchTypes .= 'ssss';
    $term = '%' . $searchName . '%';
...
switch ($searchOrder) {
    case 'name_asc': $searchSql .= ' ORDER BY up.last_name ASC, up.first_name ASC'; break;
    case 'points_desc': $searchSql .= ' ORDER BY cp.points IS NULL, cp.points DESC, up.last_name ASC'; break;
    case 'recent_desc': $searchSql .= ' ORDER BY cp.created_at DESC, up.last_name ASC'; break;
    default: $searchSql .= ' ORDER BY cp.ranking_position IS NULL, cp.ranking_position ASC, up.last_name ASC';
}
$searchSql .= ' LIMIT 30';
...
<body>
<div class="shell">
<header class="topbar"><div class="topbar-inner"><a class="brand" href="../../index.php"><img class="brand-logo" src="<?php echo h(path_from_root("assets/images/ichnos-logo.jpg")); ?>" alt="<?php echo h(APP_NAME); ?> logo"><span class="brand-copy"><strong><?php echo h(APP_NAME); ?></strong><span><?php echo h(APP_TAGLINE); ?></span></span></a><nav class="nav" aria-label="&#922;&#973;&#961;&#953;&#945; &#960;&#955;&#959;&#942;&#947;&#951;&#963;&#951;"><div class="nav-group main"><a href="../../index.php">&#913;&#961;&#967;&#953;&#954;&#942;</a><a class="active" href="searchdashboard.php">&#913;&#957;&#945;&#950;&#942;&#964;&#951;&#963;&#951;</a><?php if ($isAdmin): ?><a href="../admin/dashboard.php">Admin</a><?php endif; ?><?php if ($isCandidate): ?><a href="../candidate/candidatedashboard.php">Candidate</a><?php endif; ?></div><div class="nav-group auth"><?php if ($isGuest): ?><a class="auth-link primary" href="../../auth/register.php">&#917;&#947;&#947;&#961;&#945;&#966;&#942;</a><a class="auth-link" href="../../auth/login.php">&#931;&#973;&#957;&#948;&#949;&#963;&#951;</a><?php else: ?><a class="auth-link" href="../../auth/logout.php">&#913;&#960;&#959;&#963;&#973;&#957;&#948;&#949;&#963;&#951;</a><?php endif; ?></div></nav></div></header>
<main class="page">
